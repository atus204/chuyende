...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _autoScrollBanner() {
    ...
    if (foodProvider.banners.isEmpty) return;
    final next = (_bannerPage + 1) % foodProvider.banners.length;
  ...
  Widget build(BuildContext context) {
    ...
    final category = food.categories[_selectedCategory];
    final foods = food.getByCategory(category);
    final popular = food.getPopular();
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // ── Banner Carousel ─────────────────
                        if (food.banners.isNotEmpty) ...[
                          SizedBox(
                            height: 160,
                            child: PageView.builder(
                              controller: _bannerCtrl,
                              itemCount: food.banners.length,
                              onPageChanged: (i) =>
                                  setState(() => _bannerPage = i),
                              itemBuilder: (_, i) {
                                final b = food.banners[i];
                                final color = Color(
                                    int.parse(b['color'] ?? '0xFFFF6B35'));
                                return _buildBanner(
                                    b['title'] ?? '',
                                    b['subtitle'] ?? '',
                                    b['code'] ?? '',
                              ...
                              child: Column(
                                children: [
                                  AnimatedContainer(
                                    ...
                                    child: Center(
                                      child: Text(food.categoryEmojis[i],
                                          style:
                                  ...
                                  Text(
                                    ...
                                    style: TextStyle(
                                      fontSize: 11,
                                      fontWeight: i == _selectedCategory
                                          ? FontWeight.w600
                                          : FontWeight.w400,
                                      color: i == _selectedCategory
                                          ? AppColors.primary
                        ...
                        // ── Menu by Category ────────────────
                        _sectionHeader(
                          food.categories[_selectedCategory] == 'Tất cả'
                              ? 'Tất cả món ăn'
                              : food.categories[_selectedCategory],
                          () => Navigator.pushNamed(context, AppRoutes.menu),
                        ),
                        const SizedBox(height: 14),
                        ...foods.map(
                          (f) => FoodCard(
                            ...
                            horizontal: true,
                            onAddToCart: () {
                              context.read<CartProvider>().addItem(f);
                              _showAddedSnack(context, f);
                            },
                          ),
                        ),
  ...
  Widget _buildBanner(
      String title, String subtitle, String code, Color color) {
    return Container(
